...
async function webhookInfinitePay(request) {
  try {
    ...
    console.log('Venda confirmada (Saia das Dividas):', data.order_nsu, data.paid_amount);
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/verify-payment') {
      return verificarPagamento(request);
    }
    if (request.method === 'POST' && url.pathname === '/api/webhook-infinitepay') {
      return webhookInfinitePay(request);
    }

    // Qualquer outra rota: serve o arquivo estático normalmente
